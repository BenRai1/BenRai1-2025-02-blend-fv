...
//------------------------------- RULES OK START ------------------------------------
    ...
    // execute_donate() reverts if current_contract does not have allowance by from for amount 
    #[rule]
    pub fn donate_no_allowance(e: &Env) {
        let from: Address = nondet_address();
        let amount: i128 = nondet();
        let pool_address: Address = nondet_address();
        ...
        //setup
        let allowance: i128 = cvlr::nondet();
        cvlr_assume!(allowance < amount);
        unsafe {GHOST_ALLOWANCE = allowance};
    ...
    // execute_donate() reverts if from does not have balance of amount 
    #[rule]
    pub fn donate_no_balance(e: &Env) {
        let from: Address = nondet_address();
        let amount: i128 = nondet();
        let pool_address: Address = nondet_address();
        cvlr_assume!(from != e.current_contract_address());
        ...
        cvlr_assume!(from_balance_before < amount);
